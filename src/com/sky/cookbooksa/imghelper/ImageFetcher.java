...
package com.sky.cookbooksa.imghelper;
...
import com.sky.cookbooksa.BuildConfig;
...
public class ImageFetcher extends ImageResizer {
	...
	private void checkConnection(Context context) {
		...
		if (networkInfo == null || !networkInfo.isConnectedOrConnecting()) {
			Toast.makeText(context, "亲，请检测网络.", Toast.LENGTH_LONG).show();
...
//			Bitmap  bitmap = BitmapFactory.decodeResource(this.getContext().getResources(), R.drawable.test);
			Bitmap bitmap = BitmapFactory.decodeStream(getClass().getResourceAsStream("/res/drawable/newindex_loaderr.png"));
			return bitmap;
		}
//		return null;
	}

	public static Bitmap getCache(String data) {
		try {
			final File cacheDir = DiskLruCache.getDiskCacheDir(mContext,
					HTTP_CACHE_DIR);

			final DiskLruCache cache = DiskLruCache.openCache(mContext,
					cacheDir, HTTP_CACHE_SIZE);
		...
		try {
			...
			try {
				String urls = urlString
						.substring(0, urlString.lastIndexOf("/"));
				String c = java.net.URLEncoder.encode(urlString.substring(urls
						.length() + 1));
				urlString = urls + "/" + c;
			} catch (Exception ex) {
			}
